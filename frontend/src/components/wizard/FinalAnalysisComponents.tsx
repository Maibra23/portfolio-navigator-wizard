...
export const PerformanceSummaryCard = ({
  ...
  let selected: { expected_return?: number; risk?: number; sharpe_ratio?: number } | undefined;
      ...
      <CardContent className="space-y-6">
            ...
            return (
              <div key={factor.name} className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-sm font-bold ${getScoreColor(factor.score)}`}>{factor.score.toFixed(0)}/100</span>
                </div>
                <div className="relative h-2 bg-gray-100 rounded-full overflow-hidden">
                  <div
                    className="absolute top-0 left-0 h-full rounded-full transition-all duration-500"
                    style={{ width: `${factor.score}%`, backgroundColor: getScoreBarColor(factor.score) }}
                  />
                </div>
        ...
        {isTriple && tripleQuality.market != null && (
          <div className="pt-4 border-t border-gray-200">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className={`p-3 rounded-lg border-2 ${selectedPortfolio === 'current' ? 'border-red-500 bg-red-50' : 'border-gray-200'}`}>
                <div className="text-xs text-gray-600 mb-1">Current</div>
                <div className="text-lg font-bold text-red-600">{tripleQuality.current.composite_score.toFixed(0)}</div>
              </div>
              <div className={`p-3 rounded-lg border-2 ${selectedPortfolio === 'weights' ? 'border-blue-500 bg-blue-50' : 'border-gray-200'}`}>
              ...
              <div className={`p-3 rounded-lg border-2 ${selectedPortfolio === 'market' ? 'border-green-500 bg-green-50' : 'border-gray-200'}`}>
...
export const MonteCarloCard = ({
  ...
  const toggleVisibility = (portfolio: 'current' | 'weights' | 'market') => {
    setReturnScenarioVisibility((prev) => ({ ...prev, [portfolio]: !prev[portfolio] }));
  };
      ...
      <CardContent className="space-y-6">
        <div className="grid grid-cols-3 gap-4">
          <div className="p-4 rounded-lg bg-red-50 border border-red-200">
            ...
            <div className="text-2xl font-bold text-red-800">{(var95 * 100).toFixed(1)}%</div>
          ...
          <div className="p-4 rounded-lg bg-blue-50 border border-blue-200">
            ...
            <div className="text-2xl font-bold text-blue-800">{(expectedReturn * 100).toFixed(1)}%</div>
          ...
          <div className="p-4 rounded-lg bg-green-50 border border-green-200">
            ...
            <div className="text-2xl font-bold text-green-800">{(bestCase * 100).toFixed(1)}%</div>
          ...
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart margin={{ top: 10, right: 20, bottom: 30, left: 10 }}>
                <defs>
                  <linearGradient id="fa-colorGradient-red" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#ef4444" stopOpacity={0.8} />
                    <stop offset="95%" stopColor="#ef4444" stopOpacity={0.1} />
                  </linearGradient>
                  <linearGradient id="fa-colorGradient-blue" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#3b82f6" stopOpacity={0.8} />
                    <stop offset="95%" stopColor="#3b82f6" stopOpacity={0.1} />
                  </linearGradient>
                  <linearGradient id="fa-colorGradient-green" x1="0" y1="0" x2="0" y2="1">
                ...
                <RechartsTooltip
                  ...
                  labelFormatter={(label: number) => `Return: ${Number(label).toFixed(1)}%`}
                />
                <Legend verticalAlign="top" height={36} />
                {isTriple && returnScenarioVisibility.current && tripleMonteCarlo.current.histogram_data && (
                  <Area
                    type="monotone"
                ...
                {isTriple && returnScenarioVisibility.weights && tripleMonteCarlo.weights.histogram_data && (
                  <Area
                    type="monotone"
                ...
                {isTriple && returnScenarioVisibility.market && tripleMonteCarlo.market?.histogram_data && (
                  <Area
                    type="monotone"
                ...
                {!isTriple && selectedMonteCarlo.histogram_data && (
                  <Area
                    ...
                    strokeWidth={2}
                    fill={selectedColor === 'red' ? 'url(#fa-colorGradient-red)' : selectedColor === 'blue' ? 'url(#fa-colorGradient-blue)' : 'url(#fa-colorGradient-green)'}
                  />
                )}
                {selectedReturnScenario && (
                  <ReferenceLine
                    x={(selectedMonteCarlo.percentiles?.[selectedReturnScenario as 'p5' | 'p25' | 'p50' | 'p75' | 'p95'] ?? 0) * 100}
                    stroke={selectedStroke}
                    strokeWidth={3}
                    strokeDasharray="5 5"
                  />
          ...
          <div className="flex flex-wrap gap-2 justify-start border-b pb-3">
            <button
              ...
              className={`flex items-center gap-2 px-3 py-1.5 rounded-md border text-xs transition-all ${
                returnScenarioVisibility.current ? 'border-red-300 bg-red-50 hover:bg-red-100' : 'border-gray-200 bg-gray-100 opacity-50'
              }`}
              title={returnScenarioVisibility.current ? 'Hide Current Portfolio' : 'Show Current Portfolio'}
            >
              {returnScenarioVisibility.current ? <Eye className="h-4 w-4 text-red-600" /> : <EyeOff className="h-4 w-4 text-gray-500" />}
            ...
            <button
              ...
              className={`flex items-center gap-2 px-3 py-1.5 rounded-md border text-xs transition-all ${
                returnScenarioVisibility.weights ? 'border-blue-300 bg-blue-50 hover:bg-blue-100' : 'border-gray-200 bg-gray-100 opacity-50'
              }`}
              title={returnScenarioVisibility.weights ? 'Hide Weights-Optimized' : 'Show Weights-Optimized'}
            >
              {returnScenarioVisibility.weights ? <Eye className="h-4 w-4 text-blue-600" /> : <EyeOff className="h-4 w-4 text-gray-500" />}
            ...
            {tripleMonteCarlo.market && (
              <button
                ...
                className={`flex items-center gap-2 px-3 py-1.5 rounded-md border text-xs transition-all ${
                  returnScenarioVisibility.market ? 'border-green-300 bg-green-50 hover:bg-green-100' : 'border-gray-200 bg-gray-100 opacity-50'
                }`}
                title={returnScenarioVisibility.market ? 'Hide Market-Optimized' : 'Show Market-Optimized'}
              >
                {returnScenarioVisibility.market ? <Eye className="h-4 w-4 text-green-600" /> : <EyeOff className="h-4 w-4 text-gray-500" />}
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            {scenarios.map((scenario) => {
              const scenarioKey = scenario.key as 'p5' | 'p25' | 'p50' | 'p75' | 'p95';
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 p-2 rounded-lg border-2 transition-all text-left ${
                    isSelected ? 'bg-blue-50 border-blue-400 shadow-md' : 'bg-gray-50 border-gray-200 hover:border-gray-300 hover:bg-gray-100'
                  }`}
                >
                  ...
                  {isSelected && <div className="text-xs text-blue-600 font-semibold">Selected</div>}
                </button>
              );
            })}
          </div>
            ...
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent side="bottom" className="max-w-xs">
                  Probability of portfolio losses exceeding thresholds over 1 year (Monte Carlo).
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
          </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <div className="text-sm text-blue-800 space-y-1">
                <p>
                  • Probability of positive returns: <span className="font-semibold">{selectedMonteCarlo.probability_positive?.toFixed(1) ?? 0}%</span>
                </p>
                <p>
                  • Expected return range: <span className="font-semibold">{((selectedMonteCarlo.percentiles?.p25 ?? 0) * 100).toFixed(1)}%</span> to{' '}
                  <span className="font-semibold">{((selectedMonteCarlo.percentiles?.p75 ?? 0) * 100).toFixed(1)}%</span>
                </p>
                <p>
                  • Value at Risk (95% confidence): <span className="font-semibold">{(var95 * 100).toFixed(1)}%</span> maximum expected loss
                </p>
              </div>
            </div>
          </div>

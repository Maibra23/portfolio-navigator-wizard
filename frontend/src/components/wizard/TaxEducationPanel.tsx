      ...
      <CardContent className="pt-0">
        <Tabs defaultValue="isk-kf" className="w-full">
          <TabsList className="grid w-full grid-cols-3 h-9">
            <TabsTrigger value="isk-kf" className="text-xs">
              <span className="inline-flex items-center gap-1.5">
                <span className="w-5 h-5 bg-green-100 rounded-full flex items-center justify-center text-[9px] font-bold text-green-700">ISK</span>
                ISK / KF
              </span>
            </TabsTrigger>
            <TabsTrigger value="af" className="text-xs">
              <span className="inline-flex items-center gap-1.5">
                <span className="w-5 h-5 bg-purple-100 rounded-full flex items-center justify-center text-[9px] font-bold text-purple-700">AF</span>
                AF
              </span>
            </TabsTrigger>
          ...
          <TabsContent value="isk-kf" className="mt-3 space-y-3">
            ...
            <div className="bg-blue-50/60 border border-blue-200/60 rounded-lg p-3 text-xs space-y-1.5">
              ...
              <ul className="space-y-1 text-blue-800">
                <li><strong>Schablonrantan</strong> (standard interest rate) -- Set annually by the government: statslanerantan + 1 pp. For {tableTaxYear}: <strong>{schablonPctDisplay}%</strong>. Determined by Riksgalden (Swedish National Debt Office).</li>
                <li><strong>Schablonintakt</strong> (imputed income) -- A notional (deemed) income: taxable capital x schablonrantan. Not real income; it is the amount that gets taxed at 30%.</li>
                <li><strong>Skattefritt grundavdrag</strong> (tax-free threshold) -- Capital below this level is exempt from ISK/KF tax. {tableTaxYear}: {taxFreeDisplay} SEK.</li>
              </ul>
            </div>

            {/* ISK description */}
            <div className="bg-card rounded-lg border border-border p-3 space-y-2">
              <p className="text-xs font-semibold text-green-800">ISK (Investeringssparkonto) -- Investment Savings Account</p>
              <p className="text-xs text-muted-foreground">
                Uses <strong>schablonbeskattning</strong>: you pay a fixed tax based on capital value, regardless of actual gains. Tax = (Capital - tax-free threshold) x schablonrantan x 30%.
              </p>
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div>
                  ...
                  <ul className="list-disc ml-4 space-y-0.5 text-muted-foreground">
                    ...
                    <li>Tax-free level: {taxFreeDisplay} SEK ({tableTaxYear})</li>
                  </ul>
                </div>
                <div>
            ...
            <div className="bg-card rounded-lg border border-border p-3 space-y-1">
              <p className="text-xs font-semibold text-blue-800">KF (Kapitalforsakring) -- Capital Insurance</p>
              <p className="text-xs text-muted-foreground">
                Taxed <strong>identically to ISK</strong> (same schablonrantan, same tax-free levels). The key difference: KF includes <strong>life insurance protection</strong> -- beneficiaries receive the portfolio value directly, bypassing the estate. Choose KF for inheritance/estate-planning needs.
              </p>
            </div>

            {/* Guidance callout */}
            <p className="text-[11px] text-muted-foreground bg-muted/50 rounded px-2.5 py-1.5 border border-border">
              ISK/KF is generally better when your expected return exceeds the schablonrantan (~{schablonPctDisplay}%). Below the tax-free threshold ({taxFreeDisplay} SEK in {tableTaxYear}), ISK/KF tax is zero.
            </p>
          </TabsContent>
          ...
          <TabsContent value="af" className="mt-3 space-y-3">
            <div className="bg-card rounded-lg border border-border p-3 space-y-2">
              <p className="text-xs font-semibold text-purple-800">AF (Aktie- och Fondkonto) -- Stock & Fund Account</p>
              <p className="text-xs text-muted-foreground">
                Uses <strong>traditional capital gains taxation</strong>: 30% tax on realized gains (when you sell) and on dividends. No tax until you sell.
              </p>
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div>
                  <p className="font-medium text-green-700 mb-0.5">Advantages</p>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-2.5 text-[11px]">
              <p className="font-semibold text-amber-900 mb-0.5">Note for this wizard</p>
              <p className="text-amber-800">
                Our projections assume quarterly rebalancing, which generates taxable events each quarter in AF accounts. Tax is estimated from expected annual returns.
              </p>
            </div>

            <p className="text-[11px] text-muted-foreground bg-muted/50 rounded px-2.5 py-1.5 border border-border">
              AF is generally better for long-term buy-and-hold with infrequent trading, or when you expect returns lower than the schablonrantan and want to defer tax.
            </p>
          </TabsContent>
          ...
          <TabsContent value="compare" className="mt-3 space-y-2">
            <p className="text-[11px] text-muted-foreground">
              ISK/KF: fixed annual tax (schablonbeskattning); AF: 30% of gains. Adjust capital, tax year or returns below. Max {MAX_RETURN_ROWS} return rows.
            </p>
              ...
              <table className="w-full text-[11px] border-collapse">
                ...
                <tbody>
                  {rows.map((row, i) => {
                    ...
                    const bestBadgeColor = row.bestAccount === 'AF'
                    ...
                    return (
                      <tr key={`${row.pct}-${i}`} className={i % 2 === 0 ? 'bg-card' : 'bg-muted/30'}>
                        ...
                        <td className="p-1.5 text-center">
                          <span className={`inline-block rounded px-1.5 py-0.5 text-[10px] font-semibold ${bestBadgeColor}`}>
                            {row.bestAccount}
                          </span>
                        </td>
                      </tr>
                ...
                <Collapsible className="group mt-1 rounded-md border border-border">
                  ...
                  <CollapsibleContent>
                    <div className="border-t border-border bg-muted/20 px-3 py-2 text-[11px] text-muted-foreground space-y-2">
                      <p className="font-medium text-foreground">Capital = {tableCapital.toLocaleString('sv-SE')} SEK, Tax year {tableTaxYear} (skattefritt grundavdrag: {taxFreeLevel.toLocaleString('sv-SE')} SEK), schablonrantan: {schablonPct}%.</p>
                      <p><strong>ISK / KF:</strong> Taxable = {tableCapital.toLocaleString('sv-SE')} - {taxFreeLevel.toLocaleString('sv-SE')} = {taxableCapital.toLocaleString('sv-SE')} SEK. Schablonintakt (imputed income) = {taxableCapital.toLocaleString('sv-SE')} x {schablonPct}% = {imputedIncome.toLocaleString('sv-SE')} SEK. Tax = {imputedIncome.toLocaleString('sv-SE')} x 30% = <strong>{iskTax.toLocaleString('sv-SE')} SEK</strong> (same every row).</p>
                      <p><strong>AF at {maxReturnPct}%:</strong> Gain = {tableCapital.toLocaleString('sv-SE')} x {maxReturnPct}% = {gainAtMax.toLocaleString('sv-SE')} SEK. Tax = 30% x {gainAtMax.toLocaleString('sv-SE')} = <strong>{afTaxAtMax.toLocaleString('sv-SE')} SEK</strong>.</p>
                      <p><strong>Savings:</strong> For each row: highest tax - lowest tax = the amount saved by choosing the best account.</p>
                    </div>
                  </CollapsibleContent>
                </Collapsible>
              );

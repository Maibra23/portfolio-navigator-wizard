    ...
    const generateRecommendations = async () => {
      ...
      try {
        ...
        if (courtagClass && transactionCosts) {
          ...
          const validResults = courtageResults.filter(
            (r) => r !== null,
          ) as any[];

          if (validResults.length > 0) {
            const currentTotal =
              transactionCosts.totalFirstYearCost +
              transactionCosts.annualRebalancingCost * 4;
            const bestOption = validResults.reduce((best, curr) => {
              const currTotal =
                curr.totalFirstYear + curr.annualRebalancing * 4;
              const bestTotal = best
                ? best.totalFirstYear + best.annualRebalancing * 4
                : Infinity;
            ...
            if (bestOption) {
              const bestTotal =
                bestOption.totalFirstYear + bestOption.annualRebalancing * 4;
        ...
        if (
          (accountType === "ISK" || accountType === "KF") &&
        ...
        if (
          ...
          const afTax = potentialGains * 0.3;
      ...
      const withSavings = recs.filter(
        (r) => r.savings != null && r.savings > 0,
      );
      const sorted = [...withSavings].sort(
  ...
  if (recommendations.length === 0) {
    return (
      <Card className="border border-border bg-card">
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground">
            Your settings look well-optimized. No suggestions right now.
          </p>
        </CardContent>
      </Card>
    );
        ...
        {recommendations.map((rec, index) => (
          ...
          >
            <div className="flex items-start gap-2">
              <span className="mt-0.5 shrink-0 [&_svg]:text-foreground">
                {rec.icon}
              </span>
              <div className="min-w-0 flex-1">
                ...
                {rec.savings != null && rec.savings > 0 && (
                  <p className="text-xs font-semibold text-green-600 dark:text-green-400 mt-2">
                    ...
                    {rec.savings.toLocaleString("sv-SE", {

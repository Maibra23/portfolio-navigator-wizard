...
export const StockSelection = ({
  ...
  const [sectorRecommendations, setSectorRecommendations] = useState<
    SectorRecommendation[]
  >([]);
  const [showDiversification, setShowDiversification] = useState(false);

  // NEW: Pure Strategy Portfolio Generation State
  const [strategyPortfolios, setStrategyPortfolios] = useState<
    PortfolioRecommendation[]
  >([]);
  const [isLoadingStrategy, setIsLoadingStrategy] = useState(false);
  const [selectedStrategy, setSelectedStrategy] =
    useState<string>("diversification");
  ...
  const [allGeneratedPortfolios, setAllGeneratedPortfolios] = useState<
    Record<string, PortfolioRecommendation[]>
  >({
    diversification: [],
    risk: [],
    return: [],
  ...
  const [portfolioValidation, setPortfolioValidation] =
    useState<PortfolioValidation>({
      isValid: false,
      totalAllocation: 0,
      stockCount: 0,
      warnings: [],
  ...
  const [originalRecommendation, setOriginalRecommendation] =
  ...
  const [lastRemovedOriginal, setLastRemovedOriginal] = useState<{
    symbol: string;
    allocation: number;
  } | null>(null);
  ...
  // Mini-lesson state
  const [twoAssetAnalysis, setTwoAssetAnalysis] =
    useState<TwoAssetAnalysis | null>(null);
  const [nvdaWeight, setNvdaWeight] = useState(50);
  const [customPortfolio, setCustomPortfolio] =
    useState<TwoAssetPortfolio | null>(null);
  const [isLoadingMiniLesson, setIsLoadingMiniLesson] = useState(false);
  const [availableAssetPairs, setAvailableAssetPairs] = useState<
    Array<{
      pair_id: string;
      ticker1: string;
      ticker2: string;
  ...
  const [metricsAbortController, setMetricsAbortController] =
  ...
  const generateRecommendations = (): PortfolioRecommendation[] => {
    ...
    const order = recommendationOrder[
      riskProfile as keyof typeof recommendationOrder
    ] || ["moderate", "conservative", "aggressive"];

    // Generate recommendations in the correct order
    return order.map((profileType, index) => {
      const template =
        portfolioTemplates[profileType as keyof typeof portfolioTemplates];
      return {
        ...template,
        name:
          template.name +
  ...
  useEffect(() => {
    ...
    if (
      lastLoadedRiskProfileRef.current !== null &&
    ...
    if (
      lastLoadedRiskProfileRef.current === riskProfile &&
      loadedRecommendationsRef.current.length > 0
    ) {
      console.log(
        "Skipping refetch - recommendations already loaded for",
        riskProfile,
      );
      ...
      if (
        ...
        customizedPortfolioRef.current.selectedIndex !== null
      ) {
        ...
        if (
          customizedPortfolioRef.current.hasUserModified &&
          customizedPortfolioRef.current.customizedStocks.length > 0
        ) {
          ...
          if (
            selectedIdx !== null &&
            selectedIdx < updatedRecommendations.length
          ) {
    ...
    const loadDynamicRecommendations = async () => {
      ...
      try {
        console.log(
          "Loading dynamic recommendations for risk profile:",
          riskProfile,
        );
        const response = await fetch(
          `/api/v1/portfolio/recommendations/${riskProfile}`,
        );
        ...
        if (response.ok) {
          ...
          const transformedRecommendations = data.map(
            (
              rec: {
                name?: string;
                description?: string;
              ...
              allocations: (rec.portfolio || []).map((a) => ({
                ...a,
                allocation: Math.round((a.allocation || 0) * 100) / 100,
              })),
          ...
          console.warn(
            "Failed to load dynamic recommendations, preserving existing ones if available",
          );
    ...
    try {
      ...
      const cached =
        typeof window !== "undefined"
          ? window.localStorage.getItem(MINI_LESSON_CACHE_KEY)
          : null;
      if (cached) {
        try {
          ...
          if (data && Array.isArray(data.sector_lists)) {
            ...
            const availableSectors = data.sector_lists.filter(
              (list: any) => list.assets && list.assets.length > 0,
            );
            if (availableSectors.length >= 2) {
              for (let i = 0; i < 3; i++) {
                const shuffledSectors = [...availableSectors].sort(
                  () => Math.random() - 0.5,
                );
                ...
                if (
                  sector1 &&
                  sector2 &&
                  sector1.assets.length > 0 &&
                  sector2.assets.length > 0
                ) {
                  const asset1 =
                    sector1.assets[
                      Math.floor(Math.random() * sector1.assets.length)
                    ];
                  const asset2 =
                    sector2.assets[
                      Math.floor(Math.random() * sector2.assets.length)
                    ];
            ...
            if (pairsFromCache.length > 0) {
              ...
              setCurrentPair({
                ticker1: pairsFromCache[0].ticker1,
                ticker2: pairsFromCache[0].ticker2,
              });
          ...
          console.warn(
            "Failed to parse cached mini-lesson assets",
            storageError,
          );
        }
      }
      ...
      for (let retryAttempt = 1; retryAttempt <= 3; retryAttempt++) {
        try {
          ...
          if (retryAttempt < 3)
            await new Promise((res) => setTimeout(res, retryAttempt * 500));
        } catch (e) {
      ...
      if (response && response.ok) {
        ...
        if (data.sector_lists && data.sector_lists.length >= 3) {
          ...
          if (availableSectors.length >= 2) {
            ...
            for (let i = 0; i < 3; i++) {
              ...
              if (!sector1 || !sector2) {
                const shuffled = [...availableSectors].sort(
                  () => Math.random() - 0.5,
                );
                sector1 = shuffled[0];
                sector2 = shuffled[1];
              ...
              if (
                ...
                const shortSector1 =
                  asset1Sector.length > 12
                    ? asset1Sector.substring(0, 12) + "..."
                    : asset1Sector;
                const shortSector2 =
                  asset2Sector.length > 12
                    ? asset2Sector.substring(0, 12) + "..."
                    : asset2Sector;
          ...
          if (pairs.length < 3) {
            ...
            const techList = data.sector_lists.find(
              (list) => list.list_id === "tech_growth",
            );
            const healthList = data.sector_lists.find(
              (list) => list.list_id === "healthcare_pharma",
            );

            if (
              techList &&
              healthList &&
              techList.assets.length > 0 &&
              healthList.assets.length > 0
            ) {
              const techAsset =
                techList.assets[
                  Math.floor(Math.random() * techList.assets.length)
                ];
              const healthAsset =
                healthList.assets[
                  Math.floor(Math.random() * healthList.assets.length)
                ];
            ...
            const consumerList = data.sector_lists.find(
              (list) => list.list_id === "consumer_discretionary",
            );
            const energyList = data.sector_lists.find(
              (list) => list.list_id === "energy_utilities",
            );

            if (
              consumerList &&
              energyList &&
              consumerList.assets.length > 0 &&
              energyList.assets.length > 0
            ) {
              const consumerAsset =
                consumerList.assets[
                  Math.floor(Math.random() * consumerList.assets.length)
                ];
              const energyAsset =
                energyList.assets[
                  Math.floor(Math.random() * energyList.assets.length)
                ];

              // Use individual asset sectors for descriptions
              const consumerSector =
                consumerAsset.sector || "Consumer Discretionary";
            ...
            const financialList = data.sector_lists.find(
              (list) => list.list_id === "financial_services",
            );
            const stableList = data.sector_lists.find(
              (list) => list.list_id === "stable_blue_chips",
            );

            if (
              financialList &&
              stableList &&
              financialList.assets.length > 0 &&
              stableList.assets.length > 0
            ) {
              const financialAsset =
                financialList.assets[
                  Math.floor(Math.random() * financialList.assets.length)
                ];
              const stableAsset =
                stableList.assets[
                  Math.floor(Math.random() * stableList.assets.length)
                ];

              // Use individual asset sectors for descriptions
              const financialSector =
                financialAsset.sector || "Financial Services";
              ...
              pairs.push({
                ...
                educational_focus:
                  "Financial & Consumer Services vs Stability & Health",
              });
            }
          }
        }
        ...
        if (pairs.length === 0) {
          ...
          if (attempt < 3) {
            console.log(
              `Retrying pair generation (attempt ${attempt + 1}/3)...`,
            );
            await new Promise((res) => setTimeout(res, attempt * 1000));
            return loadAssetPairs(attempt + 1);
          } else {
            setError(
              "Unable to generate asset pairs. Please refresh the page.",
            );
            return;
          }
        }
        ...
        if (pairs.length > 0) {
          ...
          if (
            ...
            setCurrentPair({
              ticker1: pairs[0].ticker1,
              ticker2: pairs[0].ticker2,
            });
            ...
            if (pairs.length > 1) {
              setCurrentPair({
                ticker1: pairs[1].ticker1,
                ticker2: pairs[1].ticker2,
              });
        ...
        if (attempt < 3) {
          ...
          setError(
            "Unable to load asset data from server. Please try again later.",
          );
        }
      }
    } catch (error) {
    ...
    const loadMiniLesson = async () => {
      ...
      try {
        ...
        if (
          ...
          console.log("Skipping mini-lesson load due to invalid/same tickers", {
            ticker1,
            ticker2,
          });
        ...
        if (response.ok) {
          ...
          const portfolioReturn =
            asset1Allocation * data.asset1_stats.annualized_return +
            asset2Allocation * data.asset2_stats.annualized_return;

          const portfolioRisk = Math.sqrt(
            Math.pow(
              asset1Allocation * data.asset1_stats.annualized_volatility,
              2,
            ) +
              Math.pow(
                asset2Allocation * data.asset2_stats.annualized_volatility,
                2,
              ) +
              2 *
                asset1Allocation *
                asset2Allocation *
                data.asset1_stats.annualized_volatility *
                data.asset2_stats.annualized_volatility *
                data.correlation,
          );
          ...
          if (availableAssetPairs && availableAssetPairs.length > 1) {
            const nextIndex = Math.floor(
              Math.random() * availableAssetPairs.length,
            );
            ...
            if (
              ...
              nextPair.ticker1 &&
              nextPair.ticker2 &&
              nextPair.ticker1 !== nextPair.ticker2
            ) {
              setCurrentPair({
                ticker1: nextPair.ticker1,
                ticker2: nextPair.ticker2,
              });
          ...
          setError(
            `Unable to load financial data (HTTP ${response.status}). Please try again.`,
          );
        }
      } catch (error) {
        ...
        setError(
          "Unable to load financial data. Retrying with a fresh list...",
        );
  ...
  useEffect(() => {
    if (twoAssetAnalysis) {
      ...
      const portfolioReturn =
        asset1Allocation * twoAssetAnalysis.asset1_stats.annualized_return +
        asset2Allocation * twoAssetAnalysis.asset2_stats.annualized_return;

      const portfolioRisk = Math.sqrt(
        Math.pow(
          asset1Allocation *
            twoAssetAnalysis.asset1_stats.annualized_volatility,
          2,
        ) +
          Math.pow(
            asset2Allocation *
              twoAssetAnalysis.asset2_stats.annualized_volatility,
            2,
          ) +
          2 *
            asset1Allocation *
  ...
  useEffect(() => {
    if (selectedPortfolioIndex !== null && originalRecommendation) {
      ...
      const avgReturn = selectedStocks.reduce(
        (sum, stock) => sum + (stock.allocation / 100) * 0.12,
        0,
      );
      const avgRisk = selectedStocks.reduce(
        (sum, stock) => sum + (stock.allocation / 100) * 0.2,
        0,
      );
  ...
  const addStock = (stock: StockResult) => {
    ...
    if (lastRemovedOriginal && lastRemovedOriginal.allocation > 0) {
      ...
      const sumOthers = selectedStocks.reduce(
        (sum, s) => sum + s.allocation,
        0,
      );
      const factor =
        sumOthers > 0 ? (sumOthers - defaultWeight) / sumOthers : 0;
      const resizedOthers = selectedStocks.map((s) => ({
        ...s,
        allocation: Math.max(0, s.allocation * factor),
      }));
      const newStock: PortfolioAllocation = {
        symbol: stock.symbol,
  ...
  const removeStock = (symbol: string) => {
    ...
    setLastRemovedOriginal(
      removed
        ? { symbol, allocation: removed.allocation }
        : { symbol, allocation: 0 },
    );
  ...
  const acceptRecommendation = (
    ...
    setOriginalSymbols(
      new Set((recommendation.allocations || []).map((a) => a.symbol)),
    );
    setAllocationHistory([]);
    setLastRemovedOriginal(null);

    // Initialize allocation tracking
    const total = recommendation.allocations.reduce(
      (sum, stock) => sum + stock.allocation,
      0,
    );
  ...
  const resetToOriginal = () => {
    if (originalRecommendation) {
      ...
      setOriginalSymbols(
        new Set(
          (originalRecommendation.allocations || []).map((a) => a.symbol),
        ),
      );
      setAllocationHistory([]);
      setLastRemovedOriginal(null);
  ...
  const handleNext = () => {
    ...
    if (
      activeTab !== "mini-lesson" &&
      activeTab !== "recommendations" &&
      (totalAllocation > 100 || totalAllocation < 85)
    ) {
    ...
    if (activeTab === "mini-lesson") {
      ...
      if (selectedPortfolioIndex === null && selectedStocks.length < 3) {
        setError(
          "Please select a recommended portfolio or add at least 3 tickers to create a portfolio before proceeding.",
        );
        return;
      }
  ...
  const calculateRealTimeMetrics = useCallback(async () => {
    ...
    console.log(
      "Calculating real-time metrics for portfolio:",
      selectedStocks.map((s) => `${s.symbol}:${s.allocation}%`),
    );
    ...
    try {
      ...
      if (response.ok) {
        ...
        throw new Error(
          `Failed to calculate metrics: ${response.status} ${errorText}`,
        );
      }
    } catch (error: any) {
      ...
      setError(
        "Failed to calculate portfolio metrics. Using fallback calculations.",
      );
  ...
  const calculateFallbackMetrics = useCallback(() => {
    ...
    const avgRisk = selectedStocks.reduce((sum, stock) => {
      const stockRisk = getStockHistoricalRisk(stock.symbol) || 0.2;
  ...
  const validatePortfolio = useCallback(() => {
    ...
    if (Math.abs(totalAllocation - 100) > 0.01) {
      warnings.push(
        `Total allocation is ${totalAllocation.toFixed(1)}%. Must equal 100%.`,
      );
    }
    ...
    if (stockCount < 3) {
      warnings.push(
        `Portfolio must have at least 3 stocks. Currently: ${stockCount}`,
      );
    }
    ...
    selectedStocks.forEach((stock) => {
      if (stock.allocation < 5) {
        warnings.push(
          `${stock.symbol} allocation (${stock.allocation.toFixed(1)}%) is very low`,
        );
      }
      if (stock.allocation > 50) {
        warnings.push(
          `${stock.symbol} allocation (${stock.allocation.toFixed(1)}%) is very high`,
        );
      }
    });

    const isValid =
      totalAllocation === 100 && stockCount >= 3 && warnings.length === 0;
  ...
  const generateStrategyPortfolios = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/v1/portfolio/recommendations/strategy-pure?${params}`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        },
      ...
      if (response.ok) {
        ...
        // Transform backend data to frontend format
        const transformedPortfolios = data.map(
          (
            portfolio: {
              portfolio?: PortfolioAllocation[];
              expectedReturn?: number;
            ...
            name:
              portfolio.name ||
              `${selectedStrategy.charAt(0).toUpperCase() + selectedStrategy.slice(1)} Strategy Portfolio ${index + 1}`,
            description:
              portfolio.description ||
              `Pure ${selectedStrategy} strategy portfolio optimized for your risk profile`,
            allocations: portfolio.portfolio || [],
            expectedReturn: portfolio.expectedReturn || 0.15,
            risk: portfolio.risk || 0.2,
        ...
        setSuccessMessage(
          `Generated ${transformedPortfolios.length} ${selectedStrategy} strategy portfolios! (${newCount}/2 generations used)`,
        );
        setTimeout(() => setSuccessMessage(null), 3000);
      ...
      setError(
        "Failed to generate strategy portfolios. Using fallback recommendations.",
      );
          ...
          >
              ...
              <div className="bg-muted rounded-lg p-4 border border-border">
                ...
                <div className="bg-muted/30 rounded-lg p-3 border border-border/50 mb-3">
                  <p className="text-xs text-muted-foreground">
                    Learn how combining different assets can reduce risk while
                    maintaining returns. Choose from educational asset pairs to
                    explore portfolio theory and diversification benefits.
                  </p>
                </div>
                  ...
                  <div className="mb-3">
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="text-xs font-medium text-foreground">
                        Select Asset Pair for Analysis
                      </h4>
                      <Button
                        variant="outline"
                        size="sm"
                ...
                {isLoadingMiniLesson ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <Card>
                        <CardHeader className="pb-2">
                          ...
                          <p className="text-xs text-muted-foreground mt-0.5">
                            {twoAssetAnalysis.asset1_stats.company_name ||
                              twoAssetAnalysis.ticker1}
                          </p>
                        </CardHeader>
                      ...
                      <Card>
                        <CardHeader className="pb-2">
                          ...
                          <p className="text-xs text-muted-foreground mt-0.5">
                            {twoAssetAnalysis.asset2_stats.company_name ||
                              twoAssetAnalysis.ticker2}
                          </p>
                        </CardHeader>
                        <CardContent className="space-y-1.5 pt-0">
                    ...
                    <Card>
                      <CardHeader className="pb-3">
                        ...
                        <p className="text-xs text-muted-foreground mt-1">
                          Adjust the allocation between{" "}
                          {twoAssetAnalysis.ticker1} and{" "}
                          {twoAssetAnalysis.ticker2} to see how it affects your
                      ...
                      <CardContent className="space-y-3 pt-0">
                        <div className="space-y-2">
                          <div className="flex justify-between text-xs text-muted-foreground">
                            <span>
                              {twoAssetAnalysis.ticker1} Weight:{" "}
                              <strong className="text-foreground">
                                {nvdaWeight}%
                              </strong>
                            </span>
                            <span>
                              {twoAssetAnalysis.ticker2} Weight:{" "}
                              <strong className="text-foreground">
                                {100 - nvdaWeight}%
                              </strong>
                            </span>
                      ...
                      <div className="text-xs text-muted-foreground space-y-2">
                        <div className="flex items-start gap-2">
                          ...
                          <div>
                            ...
                            Average yearly growth rate based on historical
                            performance.
                          </div>
                        </div>
                        <div className="flex items-start gap-2">
                          ...
                          <div>
                            ...
                            Measures return variation. Higher values indicate
                            more uncertainty.
                          </div>
                        </div>
                        <div className="pt-1.5 border-t border-border/30 mt-1.5 flex items-start gap-2">
                          ...
                          <div>
                            ...
                            Yahoo Finance (monthly returns, annualized).
                  ...
                  <Alert>
                    ...
                    <AlertDescription>
                      Unable to load financial data. Please try refreshing the
                      page.
                    </AlertDescription>
                  </Alert>
                )}
            ...
            <TabsContent value="recommendations" className="space-y-4">
              <div className="text-center mb-4">
                ...
                <p className="text-xs text-muted-foreground">
                  Personalized recommendations optimized for your{" "}
                  {getRiskProfileDisplay().toLowerCase()} risk profile using
                  live market data
                </p>
              </div>
                    ...
                    >
                      ...
                      <CardContent className="space-y-3 pt-0">
                        ...
                        <div>
                          ...
                          <Progress
                            value={recommendation.diversificationScore}
                            className="h-1.5"
                          />
                        </div>
                        ...
                        <div className="flex items-center justify-center py-4">
                          <div className="text-center">
                            ...
                            <div className="text-sm text-slate-400">
                              Please wait for calculation to complete
                            </div>
                          </div>
                        </div>
                      )}
            ...
            <TabsContent value="full-customization" className="space-y-4">
              <div className="text-center mb-4">
                ...
                <p className="text-xs text-muted-foreground">
                  Interactive visual analytics synchronized with your selected
              ...
              {selectedStocks.length >= 3 ? (
                ...
                <Card>
                  ...
                  <CardContent className="space-y-1.5 text-center text-xs text-muted-foreground pt-0">
                    <p>
                      Select a recommended portfolio or build a custom
                      allocation with at least three assets.
                    </p>
                    <p>
                      Your selections will power the live visualization
                      experience in this tab.
                    </p>
                  </CardContent>
                </Card>
              )}
            ...
            {activeTab === "dynamic-generation" && (
              <TabsContent value="dynamic-generation" className="space-y-4">
                <div className="text-center mb-4">
                  <h3 className="text-lg font-semibold mb-1">
                    Pure Strategy Portfolio Generation
                  </h3>
                  <p className="text-xs text-muted-foreground">
                    Explore different investment strategies in their purest
                    form. Each strategy is optimized using real market data and
                    proven algorithms.
                  </p>
                </div>
                ...
                <Card>
                  <CardHeader className="pb-3">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Choose an investment strategy to see how it performs with
                      your risk profile.
                    </p>
                  </CardHeader>
                  <CardContent className="space-y-3 pt-0">
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                          ...
                          >
                            ...
                            <p className="text-xs text-muted-foreground">
                              Low correlation, balanced sector exposure
                            </p>
                          </div>
                          <div
                            className={`p-3 border rounded-lg cursor-pointer transition-all ${
                          ...
                          >
                            ...
                            <p className="text-xs text-muted-foreground">
                              High expected return, growth focus
                            </p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="space-y-2">
                            ...
                            <div className="flex items-start gap-2">
                              ...
                              <div className="flex-1">
                                ...
                                <p className="text-amber-700 dark:text-amber-300 mb-2">
                                  ...
                                  strategy. This limit helps ensure you have a
                                  ...
                                  <ul className="list-disc list-inside space-y-0.5 text-amber-700 dark:text-amber-300">
                                    <li>
                                      ...
                                      {allGeneratedPortfolios[selectedStrategy]
                                        ?.length || 0}{" "}
                                      portfolios displayed above
                                    </li>
                                    <li>
                                      Compare their expected returns, risk
                                      levels, and diversification scores
                                    </li>
                                    <li>
                                      Select the portfolio that best matches
                                      your investment goals
                                    </li>
                                    <li>
                                      Or try a different investment strategy to
                        ...
                        {(generationCounts[selectedStrategy] || 0) === 0 && (
                          <div className="text-xs text-muted-foreground text-center">
                            You can generate portfolios up to 2 times for each
                            strategy
                          </div>
                        )}
                        ...
                        <div className="flex gap-3">
                          ...
                          >
                            ...
                            ) : (generationCounts[selectedStrategy] || 0) >=
                              2 ? (
                              ...
                              <CardContent className="space-y-3 pt-0">
                                ...
                                <div className="space-y-1.5 pt-1 border-t border-border/30">
                                  ...
                                  {portfolio.allocations.length > 3 && (
                                    <div className="text-xs text-muted-foreground">
                                      +{portfolio.allocations.length - 3} more
                                      stocks
                                    </div>
                                  )}
                                </div>

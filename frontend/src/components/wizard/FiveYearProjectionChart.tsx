...
export const FiveYearProjectionChart: React.FC<
  FiveYearProjectionChartProps
> = ({
  weights,
  capital,
  accountType,
      ...
      <Card>
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground">
            ...
            projections (optimistic, base, pessimistic) with Swedish tax and
            costs.
          </p>
        </CardContent>
      </Card>
  ...
  const annualizedReturnBase =
    (Math.pow(finalBaseValue / initialValue, 1 / 5) - 1) * 100;

  const totalGrowthOptimistic = finalOptimisticValue - initialValue;
  const totalGrowthPercentOptimistic =
    (finalOptimisticValue / initialValue - 1) * 100;

  const totalGrowthPessimistic = finalPessimisticValue - initialValue;
  const totalGrowthPercentPessimistic =
    (finalPessimisticValue / initialValue - 1) * 100;
          ...
          return (
            <div key={index} className="space-y-1">
              ...
              <div className="ml-5 text-xs space-y-0.5">
                ...
                <p className="text-muted-foreground">After taxes & costs</p>
              </div>
            </div>
          );
        })}
    ...
    <Card>
      <CardHeader>
            ...
            <TooltipContent side="top" className="max-w-sm">
              ...
              <div className="space-y-2">
                ...
                <p className="mt-2 text-muted-foreground italic">
                  All values shown are NET after deducting taxes and costs each
                  year.
                </p>
              </div>
            </TooltipContent>
        ...
        <p className="text-xs text-muted-foreground mt-1">
          Three scenarios showing your portfolio growth after Swedish taxes and
          transaction costs
        </p>
      </CardHeader>
      <CardContent className="space-y-6">
        {accountType === "AF" && (
          <p className="text-xs text-muted-foreground rounded-lg border border-amber-200 bg-amber-50/50 dark:bg-amber-950/20 dark:border-amber-800 px-3 py-2">
            We assume gains are realized each year (e.g. rebalancing); actual AF
            tax depends on when you sell.
          </p>
        )}
        {/* Summary Cards */}
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-xs text-green-600 dark:text-green-400 mt-0.5">
                    ...
                    SEK ({totalGrowthPercentOptimistic.toFixed(1)}%)
                  </p>
                </div>
              </div>
            </CardContent>
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-xs text-blue-600 dark:text-blue-400 mt-0.5">
                    ...
                    SEK ({totalGrowthPercentBase.toFixed(1)}%)
                  </p>
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-xs text-red-600 dark:text-red-400 mt-0.5">
                    ...
                    SEK ({totalGrowthPercentPessimistic.toFixed(1)}%)
                  </p>
                </div>
              </div>
            </CardContent>
        ...
        <Alert className="bg-blue-50 dark:bg-blue-950/40 border-blue-200 dark:border-blue-800">
          ...
          <AlertDescription className="text-xs text-blue-900 dark:text-blue-100">
            ...
            {(risk * 100).toFixed(1)}%) to model three possible outcomes. The{" "}
            <strong>Base scenario</strong> is most likely. Hover over the chart
            lines for year-by-year details.
          </AlertDescription>
        </Alert>
        ...
        <div className="text-xs text-muted-foreground space-y-2 border-t pt-3">
          ...
          <ul className="list-disc ml-5 space-y-0.5">
            <li>
              Annual {accountType} tax based on {taxYear} rates
            </li>
            <li>
              Initial setup costs and quarterly rebalancing costs (
              {courtageClass} courtage class)
            </li>
            <li>
              These are estimates based on historical patterns and current tax
              law
            </li>
          </ul>
        </div>

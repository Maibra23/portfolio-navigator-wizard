...
- **5-year daily price history** for accurate calculations
- **Real-time data fetching** with caching to avoid repeat calls
- **Error handling** for network issues and data unavailability
- **Rate limiting** considerations for production use

#### Caching Strategy
- **Session storage** for frontend caching
- **Backend memory cache** for asset statistics
...
#### Portfolio Router (`backend/routers/portfolio.py`)
```python
# Key endpoints implemented:
@router.get("/asset-stats")           # Fetch asset statistics
...
### 1. Mini-Lesson Data Flow
```
User opens Mini-Lesson tab
    ↓
Frontend calls /api/portfolio/two-asset-analysis
    ↓
Backend fetches NVDA and AMZN data from Yahoo Finance
    ↓
...
Frontend calls /api/portfolio/asset-search
    ↓
Backend queries Yahoo Finance for matching stocks
    ↓
Backend filters for US stocks only
    ↓
Frontend displays autocomplete results
```
...
Frontend calls /api/portfolio/efficient-frontier
    ↓
Backend calculates covariance matrix
    ↓
Backend generates 500 simulated portfolios
    ↓
Frontend displays efficient frontier chart
```
